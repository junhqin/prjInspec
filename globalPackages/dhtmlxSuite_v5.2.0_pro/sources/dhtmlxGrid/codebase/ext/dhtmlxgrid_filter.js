...
dhtmlXGridObject.prototype.getFilterElementById=function(id){
	var index = this.getColIndexById(id);
	return this.getFilterElement(this._m_order ? this._m_order[index] : index);
}
...
dhtmlXGridObject.prototype._in_header_stat_max=function(t,i,c){
	var calck=function(){
		...
		for (var j=0; j<this.rowsBuffer.length; j++){
			var v = parseFloat(this._get_cell_value(this.rowsBuffer[j],ii));
			if (!isNaN(v))
				summ=Math.max(summ,v);
		}
		
		if (summ === -Infinity) return "&nbsp;";
